...
class GitHubClientMixin(ApiClient):  # type: ignore
    ...
    def search_repositories(self, query: str) -> Sequence[JSONData]:
        # Explicitly typing to satisfy mypy.
        repositories: Sequence[JSONData] = self.get("/search/repositories", params={"q": query})
        return repositories
    ...
    def search_issues(self, query: str) -> JSONData:
    ...
    def create_token(self) -> JSONData:
        headers = {
            # TODO(jess) -> None: remove this whenever it's out of preview
            "Accept": "application/vnd.github.machine-man-preview+json",
        }
    ...
    def search_file(self, repo, filename) -> None:
